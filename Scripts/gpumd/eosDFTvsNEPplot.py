...
def main():
    ...
    folders=sorted([f for f in os.listdir(args.eos) if os.path.isdir(os.path.join(args.eos,f))],key=float)
    ...
    print(f"DFT: V0={fit_dft.v0:.4f} A3, B0={fit_dft.b0*160.2177:.2f} GPa")
    print(f"NEP: V0={fit_nep.v0:.4f} A3, B0={fit_nep.b0*160.2177:.2f} GPa")
    ...
    plt.plot(v,fit_dft(v),label="DFT EOS")
    plt.plot(v,fit_nep(v),"--",label="NEP EOS")
    ...
    np.savetxt(args.out+".txt",np.column_stack([vol,dft,nep]),
               header="Volume(A3) DFT(eV) NEP(eV)",fmt="%.6f")
